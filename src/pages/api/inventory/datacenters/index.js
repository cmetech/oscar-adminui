...
async function handler(req, res) {
  if (req.method === 'GET') {
    ...
    try {
      const response = await axios.get(`${oscarConfig.MIDDLEWARE_INVENTORY_API_URL}/datacenters`, {
      ...
      if (response?.data) {
        ...
        const filteredData = dataToFilter.filter(
          item =>
            ...
            item?.location?.toLowerCase().includes(queryLowered)
        )
    ...
    try {
      ...
      const response = await axios.post(`${oscarConfig.MIDDLEWARE_INVENTORY_API_URL}/datacenters`, data, {
        headers: {
