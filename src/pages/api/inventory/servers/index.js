...
async function handler(req, res) {
  if (req.method === 'GET') {
    ...
    try {
      const response = await axios.get(`${oscarConfig.MIDDLEWARE_INVENTORY_API_URL}/servers`, {
      ...
      if (response?.data) {
        ...
        const filteredData = dataToFilter.filter(
          item =>
            ...
            item?.hostname?.toLowerCase().includes(queryLowered) ||
            item?.datacenter_name?.toLowerCase().includes(queryLowered) ||
            item?.environment_name?.toLowerCase().includes(queryLowered) ||
