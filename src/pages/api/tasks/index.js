...
async function handler(req, res) {
  if (req.method === 'GET') {
    ...
    try {
      const response = await axios.get(`${oscarConfig.MIDDLEWARE_API_URL}/tasks`, {
      ...
      if (response?.data) {
        ...
        const filteredData = dataToFilter.filter(
          item =>
            ...
            item?.celery_job_name?.toLowerCase().includes(queryLowered) ||
            item?.type?.toLowerCase().includes(queryLowered) ||
            item?.status?.toLowerCase().includes(queryLowered) ||
            item?.owner?.toLowerCase().includes(queryLowered) ||
            item?.organization?.toLowerCase().includes(queryLowered)
        )
