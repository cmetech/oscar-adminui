...
const DynamicDialogForm = ({ open, handleClose, onSubmit, tab }) => {
  ...
  const getDynamicTitle = tabValue => {
    const mapping = {
      1: 'Add Workflow Wizard'
    }

    return mapping[tabValue] || 'Add Workflow Wizard'
  }

  const getDynamicSubTitle = tabValue => {
    const mapping = {
      1: 'Add Workflow Information'
    }
...
const WorkflowsManager = () => {
  ...
  const handleConfirmDelete = async () => {
    console.log('Deleting workflows', selectedTaskIds)
    ...
    try {
      ...
      results.forEach(result => {
        if (result.success) {
          toast.success(`Workflow ${result.taskId} deleted successfully`)
        } else {
    ...
    <Grid container spacing={6}>
      <Grid item xs={12}>
        <Box display='flex' justifyContent='space-between' alignItems='center' mb={10}>
          <Typography variant='h4'>{t('Workflow Management')}</Typography>
