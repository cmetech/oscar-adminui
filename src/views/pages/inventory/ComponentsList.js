...
import { componentIdsAtom, componentsAtom, refetchComponentTriggerAtom, timezoneAtom } from 'src/lib/atoms'
...
const ComponentsList = props => {
        ...
        return (
          ...
          >
            <Box sx={{ display: 'flex', flexDirection: 'column' }}>
              <Typography>{row?.name?.toUpperCase()}</Typography>
              <Typography
                noWrap
                variant='caption'
                sx={{
    ...
    },
    {
      ...
      renderCell: params => {
        ...
        return (
          ...
          >
            <Box sx={{ display: 'flex', flexDirection: 'column' }}>
              <Typography noWrap>{row?.details?.toUpperCase()}</Typography>
            </Box>
          </Box>
        )
      }
      ...
      >
        <DialogTitle id='alert-dialog-title'>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <IconButton size='small' onClick={handleDeleteComponentDialogClose} aria-label='close'>
              <Icon icon='mdi:close' />
            </IconButton>
          </Box>
        </DialogTitle>
        <DialogContent>
          <Box sx={{ textAlign: 'center' }}>
            <Stack direction='row' spacing={2} justifyContent='center' alignItems='center'>
              ...
              <Box>
                <Typography variant='h6'>{t('Confirm you want to delete this component?')}</Typography>
              </Box>
            </Stack>
          </Box>
        </DialogContent>
        <DialogActions>
          <Button
            variant='contained'
            size='large'
            onClick={handleDeleteComponentDialogSubmit}
            color='error'
            autoFocus
  ...
  const handleSearch = value => {
    ...
    const filteredRows = rows.filter(row => {
      ...
      const searchFields = ['id', 'name', 'subcomponent_name', 'type', 'description']
