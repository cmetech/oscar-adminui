...
import UpdateMappingNamespaceWizard from 'src/views/pages/mapping/forms/UpdateMappingNamespaceWizard'
import NoRowsOverlay from 'src/views/components/NoRowsOverlay'
import NoResultsOverlay from 'src/views/components/NoResultsOverlay'
import CustomLoadingOverlay from 'src/views/components/CustomLoadingOverlay'

import { mappingNamespacesAtom, refetchMappingNamespaceTriggerAtom, timezoneAtom } from 'src/lib/atoms'
...
const MappingNamespaceList = props => {
  // ** Hooks
  ...
  const [currentMappingNamespace, setCurrentMappingNamespace] = useState(null)
  const [mappingNamespaces, setMappingNamespaces] = useAtom(mappingNamespacesAtom)
  const [refetchTrigger, setRefetchTrigger] = useAtom(refetchMappingNamespaceTriggerAtom)
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: params => {
        ...
        let color = 'info'
        let label = `MAPPINGS (${row?.mapping_count || 0})` //mapping count from mappingnamespace json
      ...
      renderCell: params => {
        ...
        if (row.created_at) {
          humanReadableDate = formatInTimeZone(
            parseISO(row.created_at), //from json
            timezone || 'UTC',
    ...
    },
    {
      ...
      renderCell: params => {
        ...
        if (row.modified_at) {
          humanReadableDate = formatInTimeZone(
            parseISO(row.modified_at), //from_json
            timezone || 'UTC',
            'MMM d, yyyy, h:mm:ss aa zzz'
          )
        }
          ...
          >
            <Box sx={{ display: 'flex', flexDirection: 'row' }}>
              <IconButton
                ...
                aria-label='Edit'
                onClick={() => {
                  setCurrentMappingNamespace(params.row)
                  setOpenDialog(true)
                }}
                disabled={!ability.can('update', 'mappingNamespaces')}
              >
                <Icon icon='mdi:edit' />
              </IconButton>
              <IconButton
                ...
                disabled={!ability.can('delete', 'mappingNamespaces')}
              >
        ...
        <DialogTitle id='form-dialog-title'>
          <Box sx={{ display: 'flex', flexDirection: 'column' }}>
            <Typography noWrap variant='h6' sx={{ color: 'text.primary', fontWeight: 600 }}>
              {currentMappingNamespace?.name?.toUpperCase() ?? ''}
            </Typography>
            <Typography
              noWrap
              variant='caption'
        ...
        <DialogContent>
          ...
          <Box sx={{ mb: 8, textAlign: 'center' }}>
            <Typography variant='h5' sx={{ mb: 3 }}>
              Edit Mapping Namespace Information
            </Typography>
            <Typography variant='body2'>Updates to namespace information will be effective immediately.</Typography>
          </Box>
          <UpdateMappingNamespaceWizard
            currentMappingNamespace={currentMappingNamespace}
            rows={rows}
            setRows={setRows}
            onClose={handleUpdateDialogClose}
          />
        ...
        <DialogContent>
          <Box sx={{ textAlign: 'center' }}>
            <Stack direction='row' spacing={2} justifyContent='center' alignItems='center'>
              ...
              <Box>
                <Typography variant='h6'>{t('Confirm you want to delete this Mapping-Namespace?')}</Typography>
              </Box>
            </Stack>
          </Box>
        </DialogContent>
  ...
  const handleDeleteDialogSubmit = async () => {
    try {
      ...
      const endpoint = `/api/mappingnamespaces/${currentMappingNamespace.id}`
      const response = await axios.delete(endpoint, { headers })

      if (response.status === 204) {
        const updatedRows = rows.filter(row => row.id !== currentMappingNamespace.id)
        ...
        toast.success('Successfully deleted Namespace')
      }
    } catch (error) {
  ...
  const fetchData = useCallback(
    async (sort, sortColumn, filterModel) => {
      setLoading(true)
      await axios
        .get('/api/mappingnamespaces', {
          params: {}
        })
        .then(res => {
          setRowCount(res.data.total)//json
          setRows(res.data.rows)//json
          props.set_total(res.data.total)//json
          setMappingNamespaces(res.data.rows)//json
        })

      setLoading(false)
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [setMappingNamespaces, setRows]
  )
...
export default MappingNamespaceList
