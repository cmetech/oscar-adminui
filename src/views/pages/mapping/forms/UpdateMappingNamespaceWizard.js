...
import { mappingNamespacesAtom, refetchMappingNamespaceTriggerAtom } from 'src/lib/atoms'

const steps = [
  {
    title: 'Namespace Information',
    subtitle: 'Edit Mapping Namespace Information',
    description: 'Edit The Namespace To Which Mappings Belong'
  },
  {
    title: 'Review',
    subtitle: 'Review and Submit',
    description: 'Review the Mapping-Namepsace details and submit.'
  }
]
...
const UpdateMappingNamespaceWizard = ({ onClose, ...props }) => {

  // ** States
  const [mappingNamespaceName, setMappingNamespaceName] = useState(props?.currentMappingNamespace?.name || '')//the json prop from the list
  const [mappingNamespaceDescription, setMappingNamespaceDescription] = useState(props?.currentMappingNamespace?.description || '')
  const [activeStep, setActiveStep] = useState(0)
  const [formErrors, setFormErrors] = useState({})
  const [, setMappingNamespaces] = useAtom(mappingNamespacesAtom)
  const [, setRefetchTrigger] = useAtom(refetchMappingNamespaceTriggerAtom)
  ...
  const handleNext = async () => {
    ...
    if (activeStep === steps.length - 1) {
      try {
        ...
        const payload = {
          name: mappingNamespaceName,
          description: mappingNamespaceDescription
        }

        // Update the endpoint to point to your Next.js API route
        const endpoint = `/api/mappingnamespaces/${props.currentMappingNamespace.id}`//from  MappingNamespace list 
        const response = await axios.patch(endpoint, payload, { headers })

        if (response.data) {
          const updatedMappingNamespace = response.data

          const updatedRows = props.rows.map(row => {
            if (row.id === updatedMappingNamespace.id) {
              return updatedMappingNamespace
            }
          ...
          toast.success('Mapping-Namespace details updated successfully')

          // Call onClose to close the modal
          onClose && onClose()
        }
      } catch (error) {
        console.error('Error updating Mapping-Namespace details', error)
        toast.error('Error updating Mapping-Namespace details')
      }
    }
  }
  ...
  const handleMappingNamespaceNameChange = event => {
    setMappingNamespaceName(event.target.value)
  }

  const handleMappingNamespaceDescriptionChange = event => {
    setMappingNamespaceDescription(event.target.value)
  }
            ...
            <Grid container spacing={6}>
              <Grid item sm={6} xs={12}>
                <FormControl fullWidth>
                  <TextfieldStyled
                    fullWidth
                    value={mappingNamespaceName.toUpperCase()}
                    onChange={handleMappingNamespaceNameChange}
                    label='Name'
                    error={Boolean(formErrors?.mappingNamespaceName)}
          ...
          <Fragment>
            <Grid container spacing={1}>
              <Grid item xs={4}>
                ...
                <Grid item xs={12}>
                  <Typography>
                    Name: <strong>{mappingNamespaceName.toUpperCase()}</strong>
                  </Typography>
                </Grid>
                <Grid item xs={12}>
                  <Typography>
                    Location: <strong>{mappingNamespaceDescription.toUpperCase()}</strong>
                  </Typography>
                </Grid>
              </Grid>
            </Grid>
...
export default UpdateMappingNamespaceWizard